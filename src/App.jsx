...
const REVIEWS = [
  { name: "Kwame A.", text: "Fast delivery! Got my 711Proxy 100GB in 3 mins on WhatsApp.", stars: 5 },
  { name: "David O.", text: "ProxyGet is legit. 9Proxy IPs working for my sneaker bot.", stars: 5 },
  { name: "Sarah M.", text: "Customer support is 10/10. Helped me setup LokiProxy.", stars: 5 },
  { name: "Chidi E.", text: "NovProxy 500 IPs very clean. Will buy again.", stars: 5 },
];
const LIVE_NAMES = ["Ama K.", "John D.", "Samuel T.", "Fatima L.", "Emmanuel", "Grace O.", "Yaw B.", "Chinedu"];
...
function LivePopup(){
  ...
  if(!item) return null;
  return (
    <div className="fixed bottom-6 left-6 z-50 bg-[#1e1e1e] border border-white/10 rounded-2xl p-3 flex gap-3 items-center shadow-2xl animate-[slideIn_0.5s_ease] max-w-[300px]">
      ...
      <div>
        <div className="text-[11px] text-white/60">{item.buyer} purchased</div>
        <div className="text-[13px] font-bold leading-tight">{item.name}</div>
        <div className="text-[10px] text-green-400">{item.time} • Verified ✅</div>
      </div>
...
export default function App() {
  ...
  const handlePay = (product) => {
    if (!email.includes("@")) { alert("Enter your email first"); return; }
    const amountGHS = product.currency === "GHS"? product.price : Math.ceil(product.price / NGN_TO_GHS_RATE);
    if (!window.PaystackPop) { alert("Paystack loading, try again in 2s"); return; }
    const handler = window.PaystackPop.setup({
      key: PAYSTACK_PUBLIC_KEY,
      email, amount: amountGHS*100, currency: "GHS",
      ref: `ProxyGet_${Date.now()}`,
      callback: function(res){
        window.location.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Hi ProxyGet! I paid for ${product.name} ${getDisplayPrice(product)} Ref:${res.reference} Email:${email}`)}`;
      }
    });
    handler.openIframe();
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="px-4 md:px-8 py-10 text-center max-w-4xl mx-auto">
        ...
        <p className="text-white/60 mt-3">Search for any proxy above. Instant delivery on WhatsApp.</p>
        <div className="mt-6 max-w-xl mx-auto flex flex-col gap-3">
          <div className="relative"><span className="absolute left-4 top-3.5">🔍</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search e.g. 100 IPs, 10GB, Loki, 9Proxy..." className="w-full bg-[#1a1a1a] border border-white/10 rounded-full pl-11 pr-4 py-3.5 focus:border-green-500 focus:outline-none" /></div>
          <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com for receipt" className="w-full bg-[#1a1a1a] border border-white/10 rounded-full px-6 py-3 text-center text-sm focus:border-green-500 focus:outline-none" />
          {search.length>0 && <p className="text-xs text-white/40">Showing {filtered.length} results for "{search}" - <button onClick={()=>setSearch("")} className="text-green-500">clear</button></p>}
        </div>
      </section>
      ...
      <section className="px-4 md:px-8 pb-10 grid grid-cols-1 md:grid-cols-4 gap-4 max-w-6xl mx-auto">
        {displayProducts.map(p=>(
          <div key={p.id} className="bg-[#161616] border border-white/10 rounded-2xl p-5 hover:border-green-500/50 transition">
            <div className="text-[10px] text-white/40 font-bold tracking-widest">{p.cat}</div><h3 className="font-bold mt-1 text-[15px]">{p.name}</h3>
      ...
      {search.length===0 && <div className="text-center pb-4 text-white/30 text-xs">👆 Type in search bar to see all {ALL_PRODUCTS.length} products. Currently showing popular only.</div>}

      <section className="px-4 md:px-8 py-16 bg-[#111] border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <h3 className="text-2xl font-black text-center">What Our Customers Say ⭐⭐⭐⭐⭐</h3>
          <div className="grid md:grid-cols-4 gap-4 mt-8">
            {REVIEWS.map((r,i)=><div key={i} className="bg-[#161616] border border-white/10 rounded-2xl p-5"><div className="text-yellow-400 text-sm">{"★".repeat(r.stars)}</div><p className="text-sm mt-2 text-white/80">"{r.text}"</p><p className="text-xs mt-3 font-bold text-white/50">— {r.name}</p></div>)}
          </div>
        </div>
      </section>

      <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" className="fixed bottom-6 right-6 w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center shadow-xl hover:scale-110 transition z-50">
